...
<img src="Images/parte_3.png" width="100%" />
...
Los scripts que lanzan este escenario son launch_4_host1 y launch_4_host2. Es recomendable lanzar ambos scripts al mismo tiempo en los dos hosts, para que todos los contenedores estén en ejecución cuando se realicen las pruebas.
...
This image is based on the ubuntu image, and adds the packages iputils-ping, iproute2, and a nginx server, needed for checking everything is working correctly in each scenario.

All the containers launched by the scripts use the option --network none, which means docker doesn't create a virtual interface for the container, making it unable to communicate with others through IP protocols.

Lastly, each script launches the example scenario associated to each of the parts described in this readme. The scripts must be executed with superuser permits in order for them to perform the needed iproute2 operations. The scripts automatically perform a series of checks to ensure the scenario is working correctly, whenever these checks can't be made automatically from the host executing the script, the user will be informed of the way to check that the system's behaviour is correct.
...
In this example, two containers are launched, thus, there will be three network namespaces, two for the containers and one for the bridge. VETH links are created between each of the containers and the bridge, and IP addresses are assigned (in the same subnet) to the interfaces in the containers. This diagram shows the resulting connections and the IP address of each interface:

 <img src="Images/parte_1.png" width="100%" />

This scenario is launched by the script launch_1.
